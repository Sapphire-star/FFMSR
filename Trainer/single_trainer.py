...
from recbole.utils import dict2str, set_color, get_gpu_usage, early_stopping
...
class SingleTrainer(Trainer):
    def __init__(self, config, model):
        super(SingleTrainer, self).__init__(config, model)
    ...
    def collate_fn(self, interaction):
        batch_interns = interaction['item_id_list']
        batch_label = interaction['item_id']
        batch_length = interaction['item_length']
        batch_interns, batch_label, batch_length = np.array(batch_interns), np.array(batch_label), np.array(
            batch_length)
        ...
        return [torch.tensor(batch_interns).to(self.device), torch.tensor(batch_label).to(self.device), torch.tensor(batch_length).to(self.device), torch.tensor(
            unique_items).to(self.device)]
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # interaction = self.collate_fn(interaction)
            interaction = interaction.to(self.device)
            # interaction = tuple(tensor.to(self.device) for tensor in interaction)
